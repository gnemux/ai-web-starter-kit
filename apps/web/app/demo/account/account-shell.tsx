...
export async function getDemoAccountPageContext(nextPath = "/demo/account") {
  const locale = await getRequestLocale();
  const copy = getDictionary(locale);
  const accountResult = await getCurrentAccount();

  if (!accountResult.ok) {
    redirect(`/demo/login?next=${encodeURIComponent(nextPath)}`);
  }
...
type DemoAccountPageContext = Awaited<
  ReturnType<typeof getDemoAccountPageContext>
>;

export function DemoAccountAppShell({
  activeNav,
  children,
  context
}: {
  ...
  children: ReactNode;
  context: DemoAccountPageContext;
}) {
  return (
    <AppShell
      action={
      ...
      brand={<BrandMark subtitle={context.copy.demo.subtitle} />}
...
export function DemoAccountPageHeader({
  description,
  eyebrow,
  title
}: {
