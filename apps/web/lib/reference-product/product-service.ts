...
export type ReferenceCareSubmission = {
  id: string;
  ownerId: string;
  planId: string;
  taskId: string | null;
...
export type ReferenceCarePlan = {
  ...
  createdAt: string;
  updatedAt: string;
  tasks: ReferenceCareTask[];
  submissions: ReferenceCareSubmission[];
};

export type ReferenceProductWorkspace = {
  cats: ReferenceCat[];
...
export async function getReferenceProductWorkspace(): Promise<
  ServiceResult<ReferenceProductWorkspace>
> {
...
export async function createReferenceCatFromFormData(
  formData: FormData
): Promise<ServiceResult<ReferenceCat>> {
...
export async function createReferenceCarePlanFromFormData(
  formData: FormData
): Promise<ServiceResult<ReferenceCarePlan>> {
...
export async function publishReferenceCarePlan(
  planId: string
): Promise<ServiceResult<ReferenceCarePlan>> {
  const clientResult = await createSupabaseServerClient();
...
async function getAuthenticatedOwnerId(
  ...
  if (error || !userId) {
    return serviceError(
      "unauthorized",
      "Sign in before using the Reference Product."
    );
  }
...
function mapCat(row: CatRow): ReferenceCat {
  return {
    id: row.id,
...
function mapPlan(row: CarePlanRow): Omit<ReferenceCarePlan, "tasks" | "submissions"> {
  return {
    id: row.id,
    ownerId: row.owner_id,
    catId: row.cat_id,
...
function mapTask(row: CareTaskRow): ReferenceCareTask {
  return {
    id: row.id,
    planId: row.plan_id,
    title: row.title,
...
function mapSubmission(row: CareSubmissionRow): ReferenceCareSubmission {
  return {
    id: row.id,
    ownerId: row.owner_id,
    planId: row.plan_id,
...
function mapSupabaseError(error: { code?: string }): ServiceResult<never> {
  if (error.code === "42501") {
    return serviceError(
      "forbidden",
      "This account does not have access to that Reference Product data."
    );
  }
  ...
  return serviceError(
    "system_error",
    "The Reference Product service is temporarily unavailable."
  );
}
