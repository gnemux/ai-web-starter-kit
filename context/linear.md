...
└── Parent issues: GNE-71, GNE-72, GNE-148, GNE-73, GNE-167
...
- Stage view: `MVP1`, `MVP2`, and `MVP3` explain what each delivery stage proves.
...
└── GNE-167 MVP2/MVP4 INTEGRATIONS-00
...
`GNE-168` records the detailed MVP1-MVP3 mapping. `GNE-167` is now the MVP2/MVP4 integrations entry: MVP2 covers provider matrix, env naming, mock/no-op/sandbox strategy, and interface boundaries; MVP4 later turns those reservations into full overseas / China dual-mode operation.

Execution tasks remain as child issues under the parent issue tree below. Examples: `GNE-180` through `GNE-183` stay under `GNE-167`, and `GNE-173` through `GNE-179` stay under `GNE-171`, but those child issues should not be directly assigned to Project Milestones.
...
└── GNE-157 BILLING-06 [AI] Token allowance、credit 与 usage entitlement 模型
...
GNE-148 AI-00 [AI] AI 能力集成、模型 Provider 与 Token 计量模板
├── GNE-149 AI-01 [DOC] 定义 AI provider、model、token、credit、entitlement 边界
├── GNE-150 AI-02 [DEV] 建立 AI Provider Adapter 与模型配置结构
├── GNE-151 AI-03 [DEV] 建立 AI Service 示例：chat、completion、embedding
├── GNE-152 AI-04 [DEV] 建立 Token Usage 计量模型
...
├── GNE-180 MVP2-INT-01 [DOC] 建立 provider matrix 与阶段边界
├── GNE-181 MVP2-INT-02 [DEV] 建立 provider interface、mock/no-op adapter 与目录约定
├── GNE-182 MVP2-INT-03 [DOC/DEPLOY] 标准化 env 命名、public/secret 分层与示例模板
└── GNE-183 MVP2-INT-04 [TEST] 验证 provider 配置、no-op 行为与 secret 不泄漏

GNE-171 PRODUCT-00 [PD] Product Validation Kit / Indie 产品验证工作台
...
├── GNE-177 MVP3-CP-04 [BILLING/PAYMENT] 建立 Free/Pro 权益、checkout 与 webhook 闭环
