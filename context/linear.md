...
GNE-71 MVP2 BILLING-00 [BILLING] 计费、订阅与权益模型
├── GNE-91 BILLING-01 [DOC][MVP2] 定义计费领域模型、状态流转与事实来源
├── GNE-191 BILLING-02 [DATA][MVP2] 建立最小 Billing 数据模型与 migration 草案
├── GNE-92 BILLING-03 [CORE/API][MVP2] 建立权益判断接口与 Billing service 边界
├── GNE-93 BILLING-04 [CONFIG][MVP2] 建立 pricing plan 配置模型
├── GNE-94 BILLING-05 [DEV][MVP2] 建立订阅状态和生命周期处理
├── GNE-157 BILLING-06 [AI][MVP2] Token allowance、credit 与 usage entitlement 模型
└── GNE-95 BILLING-07 [TEST][MVP2] 验证权益、订阅状态与账本边界
...
├── GNE-105 ANALYTICS-06 [TEST][MVP2] 验证 Production PostHog 事件接收与字段完整性（In Progress）
