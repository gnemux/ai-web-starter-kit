...
11. In `GNE-229`, keep the MVP3 target convention to `@xwlc/core`, `@xwlc/ui`, `@xwlc/platform`, and `@xwlc/db`, while current code may use `@starter/*` transition names until the rename/alias decision is explicitly made. Product-specific cat-care objects must stay outside platform packages.
