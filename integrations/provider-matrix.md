...
- MVP3 consumes these foundations in the Product Validation Kit and can use sandbox/mock/no-op paths for product validation. Payment real-provider work in MVP3 is test-mode only and must keep live payment disabled.
- MVP4 owns overseas/china adapter design, env templates, domestic mock/test-mode strategy, and launch checklists.
- MVP5 or a real vertical product owns production payment readiness and live payment decisions.
...
| Payment | MVP2 SandboxProvider foundation; no live provider integrated. | Candidate research/test mode only until production readiness. | WeChat Pay and Alipay are reserved for China adapter planning and later MVP6/real-product production readiness. | SandboxProvider first; real providers may only use test/sandbox mode before MVP5. | Define provider contract, sandbox path, payment events, PostHog events, env placeholders, and safety notes. | Define overseas/china payment adapter, env template, mock/test-mode strategy, and launch checklist. | Provider secrets and webhook secrets are server-only. | Not allowed for payment secrets or webhook secrets. Public checkout config requires explicit later approval. |
