...
- 已登录账户菜单只能包含当前真实存在的动作，例如工作台、账户设置和退出登录；不得加入 History、Bookmarks、Notifications 等尚未实现的入口。
...
- 工作区主导航只展示当前真实存在的工作区路由：工作台和账户。Public 首页可通过品牌和 footer 返回，但不作为工作区主导航项出现。
- Dashboard 主体聚焦 `demo_items` 服务读写示例；当前账户状态由 shell 和账户菜单承载，不在页面主体重复做状态卡片。
- Account 页面主体只展示当前邮箱、display name 表单和保存反馈。
