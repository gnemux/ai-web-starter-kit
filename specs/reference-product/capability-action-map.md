...
- Do not extract a generic share-token package before a second product, such as
  the travel case, proves the same contract shape.
...
| Owner creates or regenerates private share link | `share_tokens` row with hash, scope, expiry, revoked state | Required Audit event; optional Outbox notification; PostHog share event | Product-local share service owns raw-token one-time return; capability adapters receive no raw token and no token hash | `common_pattern_not_extracted` | High: bearer link safety depends on never logging raw token/hash and preserving scope filters | GNE-262/GNE-263/GNE-266 |
...
| Anonymous opens valid private link | Token resolves to plan and minimum sitter DTO | Audit page access; PostHog anonymous access event optional | Token gate resolves anonymous actor and scoped DTO; page never receives owner-only objects | `common_pattern_not_extracted`; actor/token shape aligns with `common_contract_verified` | High: no-RLS store must enforce token scope at repository layer | GNE-262/GNE-266 |
