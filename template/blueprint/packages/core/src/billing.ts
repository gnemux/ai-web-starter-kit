...
export type BillingSubscription = { id: string; ownerId: string; provider: string; providerSubscriptionId: string; planId: string; priceId: string | null; status: typeof BILLING_SUBSCRIPTION_STATUSES[number]; currentPeriodStart: string | null; currentPeriodEnd: string | null; cancelAtPeriodEnd: boolean; metadata: Record<string, unknown> };
