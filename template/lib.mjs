...
export function validateProductConfig(config) {
  ...
  if (!validate(config)) throw new Error(`Invalid product config: ${ajv.errorsText(validate.errors, { separator: "; " })}`);
  return config;
}
...
export function assertArtifactInventory(actualFiles, artifacts) {
  ...
  for (const target of ["apps/web/config/product.config.ts", "supabase/config.toml", "template-manifest.json", "template-product.json", "template-version.json"]) {
...
function secretLike(text) {
  const assignments = text.matchAll(/\b(?:SECRET|TOKEN|PASSWORD|PRIVATE_KEY|SERVICE_ROLE_KEY)\b\s*[:=]\s*["']?([^\s"']+)/g);
...
function generatedConfig(config) {
  return `// Generated by the clean-template generator. Edit template/product.json, not this file.\nexport const productConfig = ${JSON.stringify(config, null, 2)} as const;\n`;
}

function generatedSupabaseConfig(config) {
...
export async function generateCandidate({ sourceRoot, outputRoot, configFile, dryRun = false, allowDirty = false }) {
  ...
  try {
    ...
    await writeFile(path.join(temporary, "apps/web/config/product.config.ts"), generatedConfig(config));
